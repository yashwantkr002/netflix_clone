...
// api paths
const apipath={
    fatchMoviesallcatagory:`${baseurl}/genre/movie/list?api_key=${apikey}`,
    fatchMovieslist:(id)=>` ${baseurl}/discover/movie?api_key=${apikey}&with_genres=${id}`,
    trandinglist:`${baseurl}/trending/movie/week?api_key=${apikey}&language=en-US`,
...
function buldbannersection(mllist){
    ...
    div.innerHTML=`
    <h2 class="banner-title">${mllist.title.substring(0,15)}</h2>
    <p class="banner-info">Tranding in movies ${mllist.release_date}</p>
...
function fatchandBild(){
    fetch(apipath.fatchMoviesallcatagory)
    .then(res=>res.json())
    .then(res=>{
    //    console.log(res.genres)
        ...
        if(Array.isArray(categories)&& categories.length){
            ...
            categories.forEach(catogry=>{
                ...
                fatchandBildSection(apipath.fatchMovieslist(catogry.id),catogry.name);
            })
        }
